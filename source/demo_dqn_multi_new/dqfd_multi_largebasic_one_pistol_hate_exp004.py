...
REWARDS = {'living':0.0, 'healthloss':0.0, 'medkit':0.0, 'ammo':0.0, 'frag':1.0, 'dist':0.0, 'suicide':-1.0, 'kill':1.0,'death':-1.0,'enemysight':0.0, 'ammoloss':0.0}
LSTM_SIZE = 1024
N_ADV = 5
N_SEQ = 5
LAMBDA_ONE = 1.0
...
#         imitation_env = Environment(sess = sess ,name=name, agent=agent, game_instance=game_instance, network=network, start_time=starttime, end_time=end_time, random_seed=0)
        imitation_env = Environment(sess = sess ,name=name, agent=agent, game_instance=game_instance, network=network, n_step=N_STEPS, random_seed=i)
...
#     parameter_server.load_model(sess=sess, step=15, model_path="../data/demo_dqn/models/model_2019-01-29-23-22-13/model.ckpt")
