...
REWARDS = {'living':0.0, 'healthloss':0.0, 'medkit':0.0, 'ammo':0.0, 'frag':100.0, 'dist':0.0, 'suicide':-1.0, 'kill':1.0,'death':-1.0}
LSTM_SIZE = 1024
N_ADV = 5
N_SEQ = 5
LAMBDA_ONE = 1.0
...
#                 self.replay_memory.batch_update_new(tree_idx, tderror,np.array(r_adv)>0)
                self.replay_memory.batch_update_new(tree_idx, l_one,np.array(r_adv)>0)
                if self.log_server is not None:
...
#         self.loss_total = tf.reduce_mean(self.loss_one +  self.loss_n + self.loss_mergin + self.loss_l2)
        ...
        self.loss_total = tf.reduce_mean(self.loss_n+ self.loss_mergin)
...
#         _,l_one, l_n, l_mergin, l_l2, tderror_total = sess.run([self.update, self.loss_one, self.loss_n, self.loss_mergin, self.loss_l2, self.tderror_total], feed_dict)
        _,l_one,l_mergin ,tderror_total = sess.run([self.update_global_weight_params,self.loss_n,self.loss_mergin, self.tderror_total], feed_dict)
        return l_one, 0,l_mergin,0, tderror_total
...
#         parameter_server.load_model(sess=sess, step=15, model_path="models/model_temp/model_2019-01-16-15-32-54/model.ckpt")
    parameter_server.load_model(sess=sess, step=15, model_path="models/model_temp/model_2019-01-27-17-10-05/model.ckpt")
