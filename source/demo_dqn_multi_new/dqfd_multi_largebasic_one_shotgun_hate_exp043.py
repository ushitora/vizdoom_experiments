...
from vizdoom import *
import os, time, random, threading, h5py, math,pickle, shutil
...
CONFIG_FILE_PATH = "./config/large_basic_shotgun_hate.cfg"
PLAY_LOGDIR = "../data/demo_dqn/playlogs/playlog_"+DATETIME.strftime("%Y-%m-%d-%H-%M-%S")+"/"
DEMO_PATH = ["../demonstration/largebasic_shotgun_hate/demo_largebasic_shotgun01.hdf5","../demonstration/largebasic_shotgun_hate/demo_largebasic_shotgun02.hdf5"]
...
from exp_condition.condition_exp043 import *
__name__ = run_mode
...
#             self.summary_image = tf.summary.merge(s_img)
    ...
    def write_loss(self,sess, step, l_o, l_n,l_c, l_l, processtime):
...
if __name__=="learning_imitation":
    ...
    with tf.device('/gpu:0'):
        ...
        end_time = (datetime.now() + timedelta(minutes=EXPERIMENT_TIME)).timestamp()
        
        coordinator = tf.train.Coordinator()

        name = "worker_imitation"
        game_instance = GameInstanceSimpleDeathmatch(DoomGame(),name=name,n_bots=1,
                                                    config_path=CONFIG_FILE_PATH, reward_param=REWARDS, steps_update_origin=10,timelimit=2)
...
def get_batch(env, s1,action, s2_one,s2_adv,r_one, r_adv,isdemo, isterminal):
    predicted_q_adv  = env.network.get_qvalue_max_learningaction(sess,s2_adv)
    predicted_q_one = env.network.get_qvalue_max_learningaction(sess,s2_one)
    ...
    return predicted_q_one, predicted_q_adv, target_one, target_adv, action ,isweight
