...
class network_simple(object):
    ...
    def __init__(self,session,resolution,n_action, learning_rate):
        ...
        with tf.name_scope("summary"):
            ...
            tf.summary.image('conv1',tf.reshape(tf.transpose(self.conv1_relu,perm=[0,3,1,2]),[-1]+list(conv1_shape[1:3])+[1]),1)
            tf.summary.image('conv2',tf.reshape(tf.transpose(self.conv2_relu,perm=[0,3,1,2]),[-1]+list(conv2_shape[1:3])+[1]),1)
