...
class Agent(object):
    ...
    def train_network(self, sess):
        batch={'s1':[], 'actions':[], 'rewards':[], 'isterminals':[], 's2':[]}
        self.buff_pointer = N_ADV
        for i in range(N_ADV):
            batch['s1'].append(self.states_buff[i:i+N_ADV])
            batch['s2'].append(self.states_buff[i+N_ADV:i+N_ADV+N_ADV])
            batch['actions'].append(self.actions_buff[i+N_ADV-1])
            batch['isterminals'].append(self.isterminals_buff[i+N_ADV-1])
            batch['rewards'].append(sum([r*GAMMA**j for j,r in enumerate(self.rewards_buff[i+N_ADV-1: i+N_ADV-1+N_ADV])]))
            
        v = np.reshape(self.network.get_value(sess, batch['s1']), [-1])
        batch['rewards'] = batch['rewards'] + v*(np.ones((N_ADV,)) - batch['isterminals'])
        ...
        return self.network.update_parameter_server(sess, batch['s2'], batch['actions'], batch['rewards'], batch['isterminals'])
    ...
    def clear_batch(self):
        self.states_buff = np.zeros((N_ADV*3-1,) + RESOLUTION, dtype=np.float32)
...
environments[0].agent.buff_pointer


# In[ ]:


environments[0].agent.push_batch(np.ones(RESOLUTION)*0.5, 2, 100, True)
environments[0].agent.buff_pointer
...
loss,batch =environments[0].agent.train_network_terminal(sess)
...
isterminal = batch['isterminals']


# In[ ]:


environments[0].agent.rewards_buff


# In[ ]:


environments[0].agent.actions_buff


# In[ ]:


environments[0].agent.isterminals_buff
...
parameter_server.weights_params
...
#     MODEL_PATH = "./models/copy_params/model_a3c_rnn_copy_181105/model.ckpt"
    ...
    with tf.device('/gpu:0'):
        ...
        name = "test"
        game_instance=GameInstanceBasic(DoomGame(), name=name, config_file_path=CONFIG_FILE_PATH,rewards=REWARDS, n_adv=N_ADV)
        network = NetworkLocal(name, parameter_server)
        agent = Agent(network)
        test_env = Environment(sess = sess ,name=name, agent=agent, game_instance=game_instance, network=network, start_time=starttime, end_time=end_time)
    ...
    parameter_server.load_model(sess=sess, model_path=MODEL_PATH, step=11)
