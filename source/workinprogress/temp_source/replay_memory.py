...
#         self.data_name = data_name
    ...
    def sample_uniform(self, n):
        idx = np.random.randint(len(self.tree), size=n)
        states = []
        actions = []
        rewards =[]
        isterminal = []
        for i in idx:
            states.append(self.tree.data[i][0])
            actions.append(self.tree.data[i][1])
            rewards.append(self.tree.data[i][2])
            isterminal.append(self.tree.data[i][3])
        return np.array(states), np.array(actions), np.array(rewards), np.array(isterminal)
