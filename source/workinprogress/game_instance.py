...
class GameInstance(object):
    ...
    def get_reward(self, m_frag, m_death, m_health, m_ammo, m_posx, m_posy):
        ...
        if self.something is not None:
            self.something.append([m_posx, m_posy])
...
#         ret_detail['dist'] = int((math.sqrt((m_posx)**2 + (m_posy)**2))/self.dist_unit) * (self.rewards['dist'] * self.dist_unit)
        ret_detail['dist'] = (math.sqrt((m_posx)**2 + (m_posy)**2)) * self.rewards['dist']
...
class GameInstanceBasic(object):
    def __init__(self, game, name="Default", config_file_path="Default",n_adv=4):
        self.name = name
        self.game = self.initialize_game(game, config_file_path)
    ...
    def make_action(self ,action, frame_repeat):
        r = self.game.make_action(action, frame_repeat)
        self.adv += (self.adv + 1) % self.n_adv
        return r,{"total":r}
