...
#     parameter_server.load_model(sess=sess, step=15, model_path="../data/demo_dqn/models/model_2019-01-29-18-57-54/model.ckpt")
...
rewards = []
for i in range(len(replaymemory)):
    rewards.append(replaymemory.tree.data[i][5])
...
# data_range = range(6000,6100)
# data_range = range(6040,6050)
...
r = test_env.game.make_action(1,[0,0,1], 5)
s = test_env.preprocess(test_env.game.get_screen_buff())
plt.imshow(s)
print(r)
...
mergin_value = np.ones((len(demo_s[:3]), N_AGENT_ACTION)) * MERGIN_VALUE
mergin_value[range(len(demo_s[:3])), [0,1,2]] = 0.0
print(mergin_value)
# feed_dict = {test_env.network.target_one_:[0.0,0.0,0.0], test_env.network.state1_:demo_s[5:7], test_env.network.a_:[2,1], test_env.network.}
feed_dict = {test_env.network.state1_:demo_s[:3], test_env.network.isdemo_:[1,1,1],              test_env.network.mergin_:mergin_value, test_env.network.a_:[0,1,2]}
sess.run([test_env.network.loss_mergin, test_env.network.q_value], feed_dict)
...
r = test_env.game.make_action(1,[0,0,1], 5)
plt.imshow(test_env.preprocess(test_env.game.get_screen_buff()))
print(r)
...
np.where(np.array(demo_r_one) == 100)
...
hoge = plot_q_learning(environments[0],demo_s[0:10])


# In[ ]:


plot_loss_adv(env=environments[0], s1=demo_s,action=demo_a,s2_one=demo_s2_one, s2_adv=demo_s2_adv,              r_one=demo_r_one, r_adv=demo_r_adv ,isterminal=demo_t,isdemo=demo_d)
