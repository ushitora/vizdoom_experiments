...
LOGDIR = "./logs/logs_temp/log_"+DATETIME.strftime("%Y-%m-%d-%H-%M-%S")+"/"
MODEL_PATH =  "./models/model_temp/model_"+DATETIME.strftime("%Y-%m-%d-%H-%M-%S")+"/model.ckpt"
...
PLAY_LOGDIR = "./playlogs/playlog_"+DATETIME.strftime("%Y-%m-%d-%H-%M-%S")+"/"
...
DEMO_PATH = ["../demonstration/large_basic_pistol_hate/demo_largebasic_pistol_hate01.hdf5"]
...
REWARDS = {'living':0.0, 'healthloss':0.0, 'medkit':0.0, 'ammo':0.0, 'frag':100.0, 'dist':0.0, 'suicide':-1.0, 'kill':1.01,'death':-1.0}
LSTM_SIZE = 1024
N_ADV = 5
N_SEQ = 5
LAMBDA_ONE = 0.0
...
#         img = (img)/255.0
    ...
    def clear_obs(self):
        self.obs = {}
        self.obs['s1'] = np.zeros((N_SEQ,)+ RESOLUTION, dtype=np.float32)
        
    def push_batch(self, s1, action,s2,  reward, isterminal, isdemo):
        self.memory.append([np.copy(s1), action, np.copy(np.concatenate([self.obs['s1'][1:], [s2]], axis=0)) , reward, isterminal, isdemo])
    ...
    def __init__(self,name, parameter_server):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            with tf.variable_scope("learning_network"):
                ...
                self.q_value, self.conv1, self.conv2,self.reshape,self.rnn,self.fc1 = self._build_model(self.state1_)
            with tf.variable_scope("target_network"):
                self.state1_target_ = tf.placeholder(tf.float32,shape=(None,N_ADV)+RESOLUTION, name="state_1")
                self.q_value_target,_,_,_,_,_ = self._build_model(self.state1_target_)
...
#         maxpool2 = NetworkSetting.maxpool2(conv2)
        ...
        return q_value, conv1, conv2,reshape,rnn,fc1
    ...
    def _build_graph(self):
        ...
        self.loss_one = LAMBDA_ONE * tf.square(self.target_one_ - tf.reduce_max(self.q_value, axis=1))
        self.tderror_n = LAMBDA1 * tf.abs(self.target_n_ - tf.reduce_max(self.q_value, axis=1))
        self.loss_n = LAMBDA1 * tf.square(self.target_n_ - tf.reduce_max(self.q_value, axis=1))
...
def load_demo_old(replay_memory, demo_path):
        ...
        for e in episodes:
            ...
            for i in range(0, n_transit):
                ...
                if i == n_transit -1 :
                    ...
                    isterminal = False
                    s2_ = states_row[i+1]
                    r,r_detail = game.get_reward(frag[i+1]-frag[i], 0, health[i+1] - health[i], ammo[i+1] - ammo[i], posx[i+1] - originx, posy[i+1]-originy)
...
#                 action = np.where(action_row[i]==1)[0][0]
                ...
                memory.append([np.copy(states_buff),action, np.copy(np.concatenate([states_buff[1:], [s2_]], axis=0)), r, isterminal, True])
...
def load_demo_hate(replay_memory, demo_path):
    for demo in demo_path[:]:
        print(demo)
        file = h5py.File(demo, 'r')
        episodes = list(file.keys())[1:]
...
#             action_row = action_row[:,[2,4,6]]
            ...
            for i in range(0, n_transit):
                ...
                if i == n_transit -1 :
                    ...
                    isterminal = False
                    s2_ = states_row[i+1]
                    r = REWARDS['living'] + REWARDS['healthloss']*(health_row[i+1] - health_row[i])*-1
...
if __name__=="learning_imitation":
    ...
    load_demo(replaymemory, DEMO_PATH)
    ...
    with tf.device('/gpu:0'):
        ...
        end_time = (datetime.now() + timedelta(minutes=60*2)).timestamp()
...
#         imitation_env = Environment(sess = sess ,name=name, agent=agent, game_instance=game_instance, network=network, n_step=10000,random_seed=0)
...
#         GIF_BUFF = []
#         REWARD_BUFF = []
#         r,f,d,_,imgs,_ = test_env.test_agent(gif_buff=GIF_BUFF,reward_buff=REWARD_BUFF)
#         GIF_BUFF[0].save('gifs/test_prelearning.gif',save_all=True, append_images=GIF_BUFF[1:], optimize=False, duration=40*4, loop=0)
...
#         parameter_server.load_model(sess=sess, step=15, model_path="models/model_temp/model_2019-01-16-15-32-54/model.ckpt")
    parameter_server.load_model(sess=sess, step=15, model_path="models/model_temp/model_2019-01-21-15-31-16/model.ckpt")
...
if __name__ == "test_agent":
    ...
    with tf.device('/gpu:0'):
        parameter_server = ParameterServer(sess,LOGDIR)
        parameter_server.load_model(sess=sess, model_apath=MODEL_PATH, step=15)
