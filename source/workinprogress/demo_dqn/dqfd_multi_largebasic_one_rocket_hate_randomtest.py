...
if __name__=="learning_imitation":
    print(LOGDIR)
    replaymemory = ReplayMemory(10000, random_seed=0)
...
# environments[0].network.pull_parameter_server(sess)
# environments[0].network.copy_network_learning2target(sess)
# environments[0].game.new_episode()

# for i in range(20):
#     np.save("./weight_records/weight_02_{:02}.npy".format(i),sess.run(parameter_server.weights_params))
#     environments[0].learning_step()

# weight_01 = [np.load("./weight_records/weight_01_{:02}.npy".format(i)) for i in range(20)]
# weight_02 = [np.load("./weight_records/weight_02_{:02}.npy".format(i)) for i in range(20)]

# for i in range(20):
#     print([(w_0 == w_1).all() for w_0, w_1 in zip(weight_01[i], weight_02[i])])

# for i in range(20):
#     print("---")
#     print([(w_0 - w_1).mean() for w_0, w_1 in zip(weight_01[i], weight_02[i])])
#     print([(w_0 - w_1).std() for w_0, w_1 in zip(weight_01[i], weight_02[i])])

# np.save("./learning_record/action_2.npy",environments[0].record_action)
# np.save("./learning_record/tree_idx_2.npy",environments[0].record_treeidx)

# action1 = np.load("./learning_record/action_1.npy")
# action2 = np.load("./learning_record/action_2.npy")
# treeidx1 = np.load("./learning_record/tree_idx_1.npy")
# treeidx2 = np.load("./learning_record/tree_idx_2.npy")

# action1

# action2

# for idx1, idx2 in zip(treeidx1, treeidx2):
#     print((idx1 == idx2).all())

# for i in range(20):
    

# [(w_0 == w_1).all() for w_0, w_1 in zip(weights_params, weight_params01)]

# weight_test_learning = test_env.network.get_weights_learngin(sess=sess)
# weight_test_target = test_env.network.get_weights_target(sess=sess)

# [(w_l==w_t).all() for w_l, w_t in zip(weight_test_learning, weight_test_target)]

# weight_test_learning01 = np.load("weight_test_learning01.npy")
# weight_test_target01 = np.load("weight_test_target01.npy")

# [(w_l==w_t).all() for w_l, w_t in zip(weight_test_learning, weight_test_learning01)]


# [(w_l==w_t).all() for w_l, w_t in zip(weight_test_target, weight_test_target01)]

# np.save("weight_test_learning01.npy", weight_test_learning)
# np.save("weight_test_target01.npy", weight_test_target)
