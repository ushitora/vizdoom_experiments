...
class ReplayMemory(object):
    ...
    def __init__(self, capacity,random_seed=0, permanent_data=0):
...
#         self.data_name = data_name
    ...
    def set_permanent_data(self, n_permanent_data):
        self.permanent_data = n_permanent_data
        self.tree.permanent_data = n_permanent_data
    ...
    def sample(self, n,beta):
        ...
        if self.tree.full:
            ...
            for i in range(n):
                v = self.randomstate.uniform(pri_seg * i, pri_seg * (i + 1))
                idx, p, data = self.tree.get_leaf(v)  # note: idx is the index in self.tree.tree
                prob = p / self.tree.total_p
                if prob <=0:
                    prob = self.epsilon
                ISWeights[i] = np.power(prob/min_prob, -beta)
            ...
            for i in range(n):
                if i == 0:
                    v = self.randomstate.uniform(self.abs_err_upper, pri_seg * (i + 1))
                else:
                    v = self.randomstate.uniform(pri_seg * i, pri_seg * (i + 1))
                idx, p, data = self.tree.get_leaf(v)  # note: idx is the index in self.tree.tree
                prob = p / self.tree.total_p
    ...
    def sample_uniform(self, n):
        indice = self.randomstate.randint(len(self.tree), size=n)
        b_memory = np.empty((n, self.tree.data[0].size), dtype=object)
        isweight = np.ones((n,),dtype=np.float32)
        for i,idx in enumerate(indice):
            b_memory[i] = self.tree.data[idx]
        return indice, b_memory, isweight

    # update priority
    def batch_update(self, tree_idxes, abs_errors):
#         abs_errors[self.tree.permanent_data:] += self.epsilon
        abs_errors[np.where(tree_idxes<self.tree.permanent_data)[0]] += self.epsilon
...
#         abs_errors[:self.tree.permanent_data] += self.demo_epsilon
        abs_errors[np.where(tree_idxes>=self.tree.permanent_data)[0]] += self.demo_epsilon
        clipped_errors = np.maximum(abs_errors, self.abs_err_upper)
        ps = np.power(clipped_errors, self.alpha)
        for ti, p in zip(tree_idxes, ps):
            self.tree.update(ti, p)
     
    def batch_update_new(self, tree_idxes, abs_errors,frag_flag):
        abs_errors[np.where(tree_idxes<self.tree.permanent_data)[0]] += self.epsilon
        abs_errors[np.where(tree_idxes>=self.tree.permanent_data)[0]] += self.demo_epsilon
        abs_errors[frag_flag] += self.demo_epsilon
        clipped_errors = np.maximum(abs_errors, self.abs_err_upper)
        ps = np.power(clipped_errors, self.alpha)
        for ti, p in zip(tree_idxes, ps):
            self.tree.update(ti, p)
